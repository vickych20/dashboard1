import React from 'react';
import img3 from "../Assets/r3.png";
import img4 from "../Assets/r4.png";
import img5 from "../Assets/r5.png";
import img1 from "../Assets/r1.png";
import img6 from "../Assets/rt1.png";



const Recommened = () => {
  return (
    <div className="flex items-center justify-around mt-4 max-md:flex-cols-2 flex-wrap max-md:hidden">
      <div className="w-[210px] h-[270px] border-[2px] border-gray-200 shadow-sm">
        <img src={img1} alt="" className="w-[140px] m-auto mt-4" />
        <div className="px-4 mt-[40px]">
          <h1 className="font-bold text-[14px] text-[#393839]">$10.30</h1>
          <p className="text-[#403e42] text-[12px]">
           ttttT-shirts with multiple colors, for men
          </p>
        </div>
      </div>
      <div className="w-[210px] h-[270px] border-[2px] border-gray-200 shadow-sm">
        <img src={img3} alt="" className="w-[140px] m-auto mt-4" />
        <div className="px-4 mt-[30px]">
          <h1 className="font-bold text-[14px] text-[#393839]">$10.30</h1>
          <p className="text-[#403e42] text-[12px]">
            T-shirts with multiple colors, for men
          </p>
        </div>
      </div>
      <div className="w-[210px] h-[270px] border-[2px] border-gray-200 shadow-sm">
        <img src={img4} alt="" className="w-[140px] m-auto mt-4" />
        <div className="px-4 mt-[40px]">
          <h1 className="font-bold text-[14px] text-[#393839]">$10.30</h1>
          <p className="text-[#403e42] text-[12px]">
            T-shirts with multiple colors, for men
          </p>
        </div>
      </div>
      <div className="w-[210px] h-[270px] border-[2px] border-gray-200 shadow-sm">
        <img src={img5} alt="" className="w-[140px] m-auto mt-4" />
        <div className="px-4 mt-[40px]">
          <h1 className="font-bold text-[14px] text-[#393839]">$10.30</h1>
          <p className="text-[#403e42] text-[12px]">
            T-shirts with multiple colors, for men
          </p>
        </div>
      </div>
      <div className="w-[210px] h-[270px] border-[2px] border-gray-200 shadow-sm">
        <img src={img6} alt="" className="w-[140px] m-auto mt-4" />
        <div className="px-4 mt-[40px]">
          <h1 className="font-bold text-[14px] text-[#393839]">$10.30</h1>
          <p className="text-[#403e42] text-[12px]">
            T-shirts with multiple colors, for men
          </p>
        </div>
      </div>
      {/*         
       <div className="flex items-center justify-around">
        <div className="w-[210px] h-[270px] border-[2px] border-gray-200 shadow-sm">
          <img src={img1} alt="" className="w-[140px] m-auto mt-4" />
          <div className="px-4 mt-[40px]">
            <h1 className="font-bold text-[14px] text-[#393839]">$10.30</h1>
            <p className="text-[#403e42] text-[12px]">
              T-shirts with multiple colors, for men
            </p>
          </div>
        </div>
        <div className="w-[210px] h-[270px] border-[2px] border-gray-200 shadow-sm">
          <img src={img3} alt="" className="w-[140px] m-auto mt-4" />
          <div className="px-4 mt-[30px]">
            <h1 className="font-bold text-[14px] text-[#393839]">$10.30</h1>
            <p className="text-[#403e42] text-[12px]">
              T-shirts with multiple colors, for men
            </p>
          </div>
        </div>
        <div className="w-[210px] h-[270px] border-[2px] border-gray-200 shadow-sm">
          <img src={img4} alt="" className="w-[140px] m-auto mt-4" />
          <div className="px-4 mt-[40px]">
            <h1 className="font-bold text-[14px] text-[#393839]">$10.30</h1>
            <p className="text-[#403e42] text-[12px]">
              T-shirts with multiple colors, for men
            </p>
          </div>
        </div>
        <div className="w-[210px] h-[270px] border-[2px] border-gray-200 shadow-sm">
          <img src={img0} alt="" className="w-[140px] m-auto mt-4" />
          <div className="px-4 mt-[40px]">
            <h1 className="font-bold text-[14px] text-[#393839]">$10.30</h1>
            <p className="text-[#403e42] text-[12px]">
              T-shirts with multiple colors, for men
            </p>
          </div>
        </div>
        <div className="w-[210px] h-[270px] border-[2px] border-gray-200 shadow-sm">
          <img src={img5} alt="" className="w-[140px] m-auto mt-4" />
          <div className="px-4 mt-[40px]">
            <h1 className="font-bold text-[14px] text-[#393839]">$10.30</h1>
            <p className="text-[#403e42] text-[12px]">
              T-shirts with multiple colors, for men
            </p>
          </div>
        </div>
      </div> */}
    </div>
  );
}

export default Recommened